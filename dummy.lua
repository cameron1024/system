...
local result = a.run(function()
  if rev == nil then
    local tx, rx = a.control.channel.oneshot()
    vim.ui.input(
      { prompt = "Enter a branch name", default = "main" },
      tx
    )
    local branch = rx()
    ...
    local curl_result = rx()
    local json = vim.json.decode(curl_result.stdout)
    rev = json.sha
    local line = string.rep(" ", indent_count) .. "rev = \"" .. rev .. "\";\n"

    -- vim.fn.setreg("+", line)
    vim.notify(
      "Rev copied to clipboard",
      vim.log.levels.INFO
    )
